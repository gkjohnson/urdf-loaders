...
const results = ENTRY_POINTS.map( entry => {
	...
	const jsdoc = filterDocumented( runJsDoc( path.resolve( ROOT_DIR, entry.source ) ) );
	return { entry, jsdoc };
...
for ( const { entry, jsdoc } of results ) {
	...
	const resolveLink = name => {
		...
		if ( targetFile === entry.output ) return anchor;
	...
	// cache all members by associated class name
	const classMembers = {};
...
function runJsDoc( source ) {
	...
	const configPath = path.join( ROOT_DIR, 'jsdoc.json' );
	const configFlag = fs.existsSync( configPath ) ? `-c "${ configPath }"` : '';
	const result = execSync( `npx jsdoc -X -r ${ configFlag } "${ source }"`, { maxBuffer: 32 * 1024 * 1024 } ).toString();
