...
public class URDFParser : MonoBehaviour
{
  ...
  public static void LoadMesh(string path, System.Action<Mesh[], Material[]> done)
  {
    ...
    else if (fileType == "dae")
    {
      ...
      done(DAELoader.Load(path, ref empty, true), null);
    }
    else
    {
              ...
              if (meshNode == null)
              {
                ...
                switch (primitiveNode.Name)
                {
                  case "box":
                    ...
                    go.transform.localScale = URDFToUnityPos(TupleToVector3(primitiveNode.Attributes[0].Value));
                    break;
                  case "sphere":
                    go = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                    go.transform.localScale = Vector3.one * (float.Parse(primitiveNode.Attributes[0].Value) * 2);
                    break;
                  case "cylinder":
                    go = new GameObject();
