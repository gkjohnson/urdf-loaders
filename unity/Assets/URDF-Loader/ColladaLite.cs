          ...
          foreach (XmlNode mesh in fc.ChildNodes)
          {
            ...
            foreach (XmlNode node in mesh.ChildNodes)
            {
              if (node.Name == "source")
              {
                ...
                if (fa != null)
                {
                  sources.Add(node.Attributes["id"].Value,
                    fa.InnerText.Split(new[] {' '}, StringSplitOptions.RemoveEmptyEntries).Select(f => float.Parse(f))
                      .ToArray());
                }
              }
              ...
              else if (node.Name == "triangles")
              {
                ...
                indices = URDFParser.GetXmlNodeChildByName(node, "p").InnerText
                  .Split(new[] {' '}, StringSplitOptions.RemoveEmptyEntries).Select(i => int.Parse(i)).ToArray();
              }
            }
            foreach (DaeInput input in inputs)
            {
              ...
              else if (input.semantic == "VERTEX")
              {
                ...
                {
                  temp.Add(URDFParser.URDFToUnityPos(new Vector3(sources[vertsSource][i], sources[vertsSource][i + 1],
